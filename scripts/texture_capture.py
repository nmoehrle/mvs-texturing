...
"""
Reads in a capture directory generated by the LiDAR Capture app, convertes to mvs-texturing format and runs texturing.

By default, the output mesh will be written to <capture_folder>/texrecon/mesh.obj

The scene folder format that is input to the texturing algorithm is given by:
...
def run_texturing(scene_folder: str, in_mesh: str, out_mesh_prefix: str = None):
    ...
    textured_mesh = out_mesh_prefix + ".obj"
...
def convert_keyframes(capture_folder, output_folder, max_image_dimension):
    ...
    input_img_folder = os.path.join(
        capture_folder, 'keyframes', 'images')
